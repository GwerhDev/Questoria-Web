...
const AccountPage = () => {
  ...
  const { username, email, is_verified, role, profile_pic } = accountData.userData || {};
      ...
      <div className="bg-surface p-6 rounded-lg shadow-md mx-auto w-full flex flex-col items-center">
        ...
        {profile_pic && (
          <div className="flex justify-center mb-4">
            <img src={profile_pic} alt="Profile" className="w-24 h-24 rounded-full object-cover" />
          </div>
        )}
        <div className="space-y-2">
          <p><strong>Username:</strong> {username}</p>
          <p><strong>Email:</strong> {email}</p>
          <p><strong>Verified:</strong> {is_verified ? 'Yes' : 'No'}</p>
